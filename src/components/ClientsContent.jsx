...
const CLIENTS_I18N = {
  ...
  ru: {
    ...
    lede: 'Независимые владельцы используют RestOS, чтобы двигать строки, которые реально считаются.',
    filterAll: 'Все',
    ...
    caseEyebrow: 'Кейс',
    caseQuote: 'RestOS заменил четырёх вендоров за неделю. ФОТ снизился на 11%, фудкост — на 3 пункта.',
  ...
  uz: {
    ...
    lede: 'Mustaqil egalar RestOS bilan haqiqatdan hisoblanadigan satrlarni harakatga keltiradi.',
    filterAll: 'Barchasi',
    ...
    caseQuote: 'RestOS to\'rt vendorni bir hafta ichida almashtirdi. Ish haqi 11%, fudkost 3 punktga tushdi.',
  ...
  'uz-cyr': {
    ...
    lede: 'Мустақил эгалар RestOS билан ҳақиқатдан ҳисобланадиган сатрларни ҳаракатга келтиради.',
    filterAll: 'Барчаси',
    ...
    caseQuote: 'RestOS тўрт вендорни бир ҳафта ичида алмаштирди. Иш ҳақи 11%, фудкост 3 пунктга тушди.',
  ...
  kaa: {
    ...
    title_a: 'Sanlar',
    title_b: 'ózi aytadı.',
    lede: 'Ǵárezsiz iyeler RestOS penen haqıyqattan esaplanatuǵın satırlardı qımıldatadı.',
    filterAll: 'Barlıǵı',
    ...
    caseQuote: 'RestOS tórt vendordı bir hápte ishinde almastırdı. Aylıq 11%, fudkost 3 punktke tústi.',
    ...
    <>
      <section className="page-hero">
        <div className="wrap">
          ...
          <div className="chips">
            <button className={'chip' + (activeFilter === -1 ? ' active' : '')} onClick={() => { setActiveFilter(-1); track('filter_select', { filter: 'all' }, lang); }}>{C.filterAll}</button>
            {C.filters.map((f, i) => (
              <button key={i} className={'chip' + (activeFilter === i ? ' active' : '')} onClick={() => { setActiveFilter(i); track('filter_select', { filter: f }, lang); }}>{f}</button>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="case-card">
            ...
            <div className="case-visual">
              ...
              <div className="case-badge">
                <span>{C.caseBadge[0]}</span>
                <span>{C.caseBadge[1]}</span>
              </div>
            </div>
          </div>
        </div>
