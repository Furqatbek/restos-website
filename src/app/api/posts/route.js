...
export async function POST(req) {
  const {
    title, slug, excerpt, body, category, glyph = 'R', color = 'b1',
  ...
  // Keyword-rich URL slug — provided or transliterated from the title, unique.
  ...
  const result = db.prepare(
    `INSERT INTO posts (title, slug, excerpt, body, category, glyph, color, read_time, lang, featured, status, meta_title, meta_description, keywords, published_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    title.trim(), finalSlug, excerpt?.trim() || null, body?.trim() || null, category?.trim() || null,
    glyph, color, read_time, lang, featured ? 1 : 0, status,
    meta_title?.trim() || null, meta_description?.trim() || null, keywords?.trim() || null, pubAt
  );
